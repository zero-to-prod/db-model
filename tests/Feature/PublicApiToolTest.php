...
function renderFixtures(): string
{
    return Api::render(
        ...
        'ZeroToProd\\DbModel\\Tests\\Fixtures\\PublicApi',
    );
}
...
it('omits internal classes from the packages own public api', function (): void {
    expect(Api::render(dirname(__DIR__, 2).'/src', 'ZeroToProd\\DbModel'))
...
it('renders a class declaration with its summary', function (): void {
    expect(renderFixtures())
        ->toContain("## ZeroToProd\DbModel\Tests\Fixtures\PublicApi\Widget\n\nA widget.\n\n```php\nfinal class Widget implements ZeroToProd\DbModel\Tests\Fixtures\PublicApi\Shape\n{");
});
...
it('renders public properties and signatures in full', function (): void {
    expect(renderFixtures())
        ...
        ->toContain('public static function tally(array &$carry, ?string $label = null, int|float $weight = 1, bool $strict = true, array $options = {"depth":1}, $extra = null, ZeroToProd\DbModel\Tests\Fixtures\PublicApi\Status $status = \ZeroToProd\DbModel\Tests\Fixtures\PublicApi\Status::Draft, string ...$tags): static;');
});
